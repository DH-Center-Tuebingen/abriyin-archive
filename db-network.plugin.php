	...
	// ========================================================================================================
		...
		foreach($TABLES as $table_name => $table) {
			...
			foreach($db->query($sql) as $row) {
				...
				$nodes[$table_name . '#' . $pk] = sprintf("{id: %s, label: %s}", 
				...
				foreach($q_single_fields as $single_field_name) {
					$edges[] = sprintf("{from: %s, to: %s, width:1, label: %s}",
						...
						json_encode($table['fields'][$single_field_name]['label'])
					);
					
					if(false&&!isset($nodes[$table['fields'][$single_field_name]['lookup']['table'] . '#' . $row[$single_field_name]]))
						$nodes[$table['fields'][$single_field_name]['lookup']['table'] . '#' . $row[$single_field_name]] = sprintf("{id: %s, label: %s}", 
							json_encode($table['fields'][$single_field_name]['lookup']['table'] . '#' . $row[$single_field_name]),
							json_encode($TABLES[$table['fields'][$single_field_name]['lookup']['table']]['item_name'] . ' #' . $row[$single_field_name]) // todo: what to display as label?
						);
				}
				...
				foreach($q_multi_fields as $multi_field_name) {
					...
					while($row = $stmt->fetch(PDO::FETCH_NUM)) {
						$edges[] = sprintf("{from: %s, to: %s, width:1, label: %s}",
							...
							json_encode($table['fields'][$multi_field_name]['label'])
						);		

						if(false&&!isset($nodes[$table['fields'][$multi_field_name]['lookup']['table'] . '#' . $row[0]]))
						$nodes[$table['fields'][$multi_field_name]['lookup']['table'] . '#' . $row[0]] = sprintf("{id: %s, label: %s}", 
							json_encode($table['fields'][$multi_field_name]['lookup']['table'] . '#' . $row[0]),
							json_encode($TABLES[$table['fields'][$multi_field_name]['lookup']['table']]['item_name'] . ' #' . $row[0]) // todo: what to display as label?
						);						
					}
				}
			}			
...
EOT;
	...
	// ========================================================================================================
	// CYTOSCAPE VERSION
	function db_network_get_js_CYTOSCAPE($div_id) {
	// ========================================================================================================
		global $TABLES;
		$nodes = array();
		$edges = array();
